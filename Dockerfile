...
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o go-websocket
...
COPY --from=builder /app/go-websocket/go-websocket .

# 把测试用的html页面也放进去
COPY --from=builder /app/go-websocket/html ./html
